...
class planszaSurface:
    ...
    def __init__(self, n_row: int, n_col: int, plansza: Plansza):
        ...
        zolty_img = pygame.transform.smoothscale(zolty_img, (self.POLE_SZEROKOSC, self.POLE_SZEROKOSC))
        czerw_img = pygame.image.load('python/media/4row_red.png')
        czerw_img = pygame.transform.smoothscale(czerw_img, (self.POLE_SZEROKOSC, self.POLE_SZEROKOSC))
        self.images = {
            Pole.zolty: zolty_img,
            Pole.czerwony: czerw_img
        }
        
        self.X_MARGIN = int((self.OKNO_SZEROKOSC - Plansza.n_col * self.POLE_SZEROKOSC) / 2)
        self.Y_MARGIN = int((self.OKNO_WYSOKOSC - Plansza.n_row * self.POLE_SZEROKOSC) / 2)
