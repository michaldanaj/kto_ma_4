...
class Silnik:
    ...
    # algorytm minimax
    def minimax(
        self, akt_gleb: int, max_gleb: int, kolor: Pole, czy_maks_gracz: bool
    ) -> float:
        # p.print()
        best_ocena: float = 0
            ...
            for j in range(self.pl.n_col):
                if self.pl.wrzuc(j, kolor):
                    ...
                    # jeśli nie, to zagłębiam się
                        ...
                        wyn = self.minimax_obc(
                            akt_gleb + 1, max_gleb, rev(kolor), True, best_ocena
                        )
                        self.pl.wyjmij(j)
    ...
    def ocen_ruchy(self, plan: Plansza, kolor: Pole):
        ...
        oceny: list[float] = [brak_ruch_punkty] * self.pl.n_col
        ...
        for j in range(self.pl.n_col):
            if self.pl.wrzuc(j, kolor):
                ...
                if wygrana:
                    ...
                    wyn = self.minimax(
                        1, self.glebokosc_anal, rev(kolor), not czy_maks_gracz
                    )
                    oceny[j] = wyn
